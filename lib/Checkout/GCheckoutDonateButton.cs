...
namespace GCheckout.Checkout {
  ...
  /// </summary>
  public class GCheckoutDonateButton : System.Web.UI.WebControls.ImageButton {
    ...
    private void SetImageUrl() {
      ...
      if (Environment == EnvironmentType.Sandbox) {
        ImageUrl = string.Format(
          "{0}://sandbox.google.com/checkout/buttons/donateNow.gif?" +
          "merchant_id={1}&w={2}&h={3}&style={4}&variant={5}",
          Protocol, MerchantID, 115, 50, StyleInUrl, VariantInUrl);
      }
      else {
        ImageUrl = string.Format(
          "{0}://checkout.google.com/buttons/donateNow.gif?" +
          "merchant_id={1}&w={2}&h={3}&style={4}&variant={5}",
          Protocol, MerchantID, 115, 50, StyleInUrl, VariantInUrl);
      }
    }
    ...
    public CheckoutShoppingCartRequest CreateRequest() {
      ...
      return new CheckoutShoppingCartRequest(MerchantID, MerchantKey, 
        Environment, _Currency, _CartExpirationMinutes, true);
    }
